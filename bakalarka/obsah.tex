...
\begin{list}{}{\setlength\parsep{0cm} \setlength\itemsep{0cm} \setlength\leftmargin{1em}}
  ...
  \item Nechť $X = \{X_1, X_2, X_3, \dots, X_n\}$ pro $n \ge 1$ je množina nevstupních symbolů v $M$.
         Pak definujme funkci $\varphi : X \rightarrow \{0,1\}^*$ tak, že $\varphi (X_i)=0^i 1$ pro $i = 1,2,3,\dots,n$. \medskip
   ...
   \item Pro každé $qA \rightarrow p b_0 B_1 b_1 B_2 b_2 \dots b_{j-1} B_{j} b_j \in R$, kde $p, q \in Q$, $j \ge 0$, $b_0,b_1,\dots,b_j \in {\Sigma}^*$ a $A, B_1,B_2,\dots,B_j \in (\Gamma - \Sigma)$, 
          přidej $<q, \varphi (A)>$ do množiny $Q_{exp}$  a do $R_{exp}$ pravidlo: \medskip

    \subitem $<q, \varphi (A)> \# \rightarrow <p, \varepsilon>' b_0 \varphi(B_1) b_1 \varphi(B_2) b_2 \dots b_{j-1} \varphi(B_{j}) b_j \#$. \medskip 

  \item Přidej do $R_{find}$ pravidla: \medskip

  \subitem $<s, 0^i> 0 \rightarrow <s, 0^{i+1}> \varepsilon $, kde $s \in Q$, $0 \le i \le n$,
  \subitem $<s, 0^i> 1 \rightarrow <s, 0^i 1> \varepsilon $, kde $s \in Q$, $0 \le i \le n$,
  \subitem $<s, 0^j 1> \# \rightarrow <s, \varepsilon> 0^j 1 \# $, kde $s \in Q$, $1 \le j \le n$, $<s, 0^j 1> \notin Q_{exp}$
  \subitem $<s, 1 > \# \rightarrow <end> \varepsilon $, kde $s \in Q$.

  \item Přidej do $R_{copy}$ pravidla: \medskip

  \subitem $<s, 0^i>' 0 \rightarrow <s, 0^{i+1}>' \varepsilon $, kde $s \in Q$, $0 \le i \le n$,
  \subitem $<s, 0^i>' 1 \rightarrow <s, 0^i 1>' \varepsilon $, kde $s \in Q$, $0 \le i \le n$,
  \subitem $<s, 0^j 1>' \# \rightarrow <s, \varepsilon>' 0^j 1 \# $, kde $s \in Q$, $1 \le j \le n$,
  \subitem $<s, 1 >' \# \rightarrow <s,\varepsilon> 1 \# $, kde $s \in Q$.
